...
- [Development guidelines](#development-guidelines)
    - [General XcelR Core](#general-xcelr-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
XcelR Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/xcelr-project/xcelr/blob/master/contrib/valgrind.supp))
...
XcelR Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 6584.
...
file descriptors it will cause problems with XcelR's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to XcelR's
